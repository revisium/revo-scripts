...
export const digestForEmittedClosure = async (
  ...
  const contents = await Promise.all(
    files.map(async (file) => ({
      file,
      bytes: Buffer.from(normalizeProviderIdentityPin(await readFile(file.path, 'utf8')), 'utf8'),
    })),
  );
...
// Provider entry modules contain their own emitted implementation pin. Hashing
// that literal makes regeneration self-referential and therefore non-stable.
const normalizeProviderIdentityPin = (source: string): string =>
  source.replace(
    /(const\s+(?:nodeGitProviderImplementationDigest|fetchGitHubProviderImplementationDigest)\s*=\s*\n?\s*)'sha256:[0-9a-f]{64}'/g,
