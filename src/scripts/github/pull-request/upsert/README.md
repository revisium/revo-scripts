...
| ------------------------------ | ------------------------------------------------ |
| Revision                       | `1`                                              |
| Operation class and operations | `write`; `github.read`, `github.write`           |
...
It performs the orchestrator pull-request creation/update operation; it does not decide readiness, merge authorization, or pipeline state. See `test/contract/github/pull-request-upsert.test.ts` and `pnpm verify`.
