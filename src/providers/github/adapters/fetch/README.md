...
Startup rejects another implementation for the same provider contract; execution selects its one registered implementation.
...
Replies carry a hashed hidden operation marker so a retry can find the prior operation; the adapter fails closed when the
